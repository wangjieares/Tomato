...
//    private int colorTriangle = Color.RED;
    ...
    private Path pathTriangle;//秒针三角形
    private Path pathMinutePointer;//分针
    private Path pathHourPointer;//时针
...
//		startNewSecondAnim();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        rotateCanvas(canvas);//转动画笔
        drawContent(canvas);
    }

    private void rotateCanvas(Canvas canvas) {
        matrixCanvas.reset();//恢复设置
    ...
    private void drawContent(Canvas canvas) {
        ...
        if (rotateSecondPointer >= 360f) {
            rotateSecondPointer %= 360f;//毫秒需要取余
        }
...
//        matrixSweepGradient.setRotate(getProgressRingRotateDegree(), centerX, centerY);
    ...
    private void initTriangleSecondPointer() {
        ...
        paintTriangle.setStyle(Paint.Style.FILL);//实心画笔
        paintTriangle.setAntiAlias(true);//抗锯齿
    ...
    private void startNewSecondAnim() {
        ...
        cancelSecondAnimIfNeed();//取消秒针动画
        updateTimePointer();

        final float startDegree = 0f;
        final float endDegree = 360f;
        secondAnim = ValueAnimator.ofFloat(startDegree, endDegree);//0-360的动画
        // FIXME 不知为何某些机型动画实际执行时间是duration的一半
        // 在构造函数里启动动画就正常，在其他方法（如onAttachedToWindow或onSizeChanged）里调用本方法就不正常
        // 出问题的机型：小米4、小米1S
        secondAnim.setDuration(60 * 1000);//60秒
        secondAnim.setInterpolator(new LinearInterpolator());//匀速开始
        secondAnim.setRepeatMode(ValueAnimator.RESTART);//value取值有RESTART，REVERSE，ValueAnimation.REVERSE 表示倒叙从新开始
        secondAnim.setRepeatCount(ValueAnimator.INFINITE);// 设置循环次数,设置为INFINITE表示无限循环
        secondAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {

            private float lastDrawValue = 0;
            private float drawInterval = 0.1f;//间歇时间是一秒

            private float lastUpdatePointerValue = 0;
            private float updatePointerInterval = 360 / 60 * 5;//30

            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                //这个是指针
                float newValue = (float) animation.getAnimatedValue();//得到当前的动画值 强转是因为它返回object 从0-360循环

                // Check if it is the time to update pointer position
                float increasedPointerValue = newValue - lastUpdatePointerValue;//现在的动画值 - 0 从1到30循环

                if (increasedPointerValue < 0) {
                    //increasedPointerValue == -330
                    increasedPointerValue = endDegree + increasedPointerValue;//动画从新开始的时候 赋值之后30
                }

                if (increasedPointerValue >= updatePointerInterval) { // >=30
                    lastUpdatePointerValue = newValue;
                    updateTimePointer();//初始化指针位置
                }

                // Check if it is the time to invalidate
                //这个是外面的圈
                float increasedDrawValue = newValue - lastDrawValue;//一秒 1-0=1 进度到360 这里就是-360
                if (increasedDrawValue < 0) {
                    //当进度到-360时候 再加上endDegree 归零
                    increasedDrawValue = endDegree + increasedDrawValue;
                }
                /**
                 * 首先初值 increasedDrawValue = 1 1-0 等于1
                 * 然后升级lastDrawValue 为1
                 * 然后newValue-lastDrawValue = 2-1 还是1
                 * 就是指针旋转1度
                 */
                if (increasedDrawValue >= drawInterval) {
                    lastDrawValue = newValue;// laseDrawValue = 1;
                    rotateSecondPointer += increasedDrawValue;//旋转角度
                    invalidate();
//					if (DEBUG) {
